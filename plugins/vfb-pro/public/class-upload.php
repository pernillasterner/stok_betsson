...
class VFB_Pro_Upload {
	...
	private function insert_attachment( $upload, $entry_id = 0 ) {
		/**
		 * Filter whether or not to skip adding uploads to media library
		 *
		 * Passing a falsey value to the filter will effectively short-circuit
		 * any attempt at adding uploads to media library.
		 *
		 * @since 3.3.1
		 *
		 */
		if ( apply_filters( 'vfbp_skip_upload_media_library', false, $this->id ) )
			return;
