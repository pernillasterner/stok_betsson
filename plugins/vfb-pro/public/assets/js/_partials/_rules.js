	...
	VFBProRuleLogic.prototype = {
		...
		compare: function( condition, fieldType ) {
			var filter         = condition.filter,
				fieldValue     = this.cleanForComparison( this.getFieldValue( condition['field-id'], fieldType, condition.value ) ),
		...
		getField: function( fieldType, columnId, conditionValue ) {
			if ( fieldType === 'radio' ) {
	            return this.getRadioField( fieldType, columnId );
	        }
	        else if ( fieldType === 'checkbox' ) {
		        return this.getCheckboxField( fieldType, columnId, conditionValue );
	        }
	        else {
	            return $( '#vfb-field-' + columnId )[0];
	        }
	    },
	    getFieldValue: function( columnId, fieldType, conditionValue ) {
		    var field = this.getField( fieldType, columnId, conditionValue ),
		    	value = '';
	    ...
	    getCheckboxField: function( fieldType, columnId, conditionValue ) {
		    var field = false,
				self  = this;
			...
			if ( $checkboxField.length ) {
				$checkboxField.each( function() {
					if ( $( this ).is( ':checked' ) ) {
						var label = self.cleanForComparison( $( this ).parent( 'label' ).text() );

						if ( self.cleanForComparison( conditionValue ) === label ) {
							field = $( this )[0];
						}
					}
				});
