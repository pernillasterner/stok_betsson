...
class VFB_Pro_Format {
	 ...
	 *
	 * Loop through all submitted data and format it in a list
	 *
	 * @access public
	 * @param mixed $value
	 * @return void
	...
	public function hidden( $form_id, $field_id, $field, $value ) {
		...
		$seq_start   = isset( $options['seq-start'] ) && !empty( $options['seq-start'] ) ? absint( $options['seq-start'] ) : 1000;
		$seq_step    = isset( $options['seq-step']  ) && !empty( $options['seq-step'] ) ? absint( $options['seq-step']  ) : 1;
		$seq_num_opt = "vfb-hidden-sequential-num-{$form_id}-{$field_id}";
		$seq_num     = get_option( $seq_num_opt );

		if ( $seq_num ) {
			// Make sure duplicate numbers are not added
			$seq_num = absint( $seq_num );
			if ( $seq_num == $value ) {
				$value = $seq_num + $seq_step;
			}
