...
class VFB_Pro_Admin_AJAX {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_vfbp-rules-autocomplete', array( $this, 'rules_autocomplete' ) );
	...
	/**
	 * Display options on Rules using Autocomplete
	 * @return [type] [description]
	 */
	public function rules_autocomplete() {
		global $wpdb;

		// Check AJAX nonce set via wp_localize_script
		check_ajax_referer( 'vfbp_ajax', 'vfbp_ajax_nonce' );

		if ( isset( $_GET['action'] ) && 'vfbp-rules-autocomplete' !== $_GET['action'] )
			return;

		$term     = esc_html( $_GET['term'] );
		...
		if ( is_array( $options ) && !empty( $options ) ) {
			foreach ( $options as $option ) {
				$all_options[] = htmlspecialchars( str_replace( '&amp;', '&', $option['label'] ) );
			}
		}

		echo wp_json_encode( $all_options );
