jQuery(document).ready(function($) {
	...
	$( document ).on( 'click', '.vfb-rules-actions-add', function( e ) {
		...
		var rules     = $( this ).closest( '.vfb-rules-container' ).find( '.vfb-rules' ),
	...
	// !Autocomplete - when the dropdown has changed, get matching options
	$( document ).on( 'change', '.vfb-rules-select-label', function () {
		$( '.vfb-rule-field-value' ).autoComplete( 'destroy' );
		vfb_rule_autocomplete( $( this ).val() );
	});

	// !Autocomplete - when input is focused, get matching options
	$( document ).on( 'focus', '.vfb-rule-field-value', function() {
		var label = $( this ).closest( 'tr' ).find( '.vfb-rules-select-label' );
		$( '.vfb-rule-field-value' ).autoComplete( 'destroy' );

		vfb_rule_autocomplete( label.val() );
	});
	 ...
	 */
	function vfb_rule_autocomplete( field ) {
		$( '.vfb-rule-field-value' ).autoComplete({
			source : function( term, suggest ) {
				$.getJSON( ajaxurl,
					{
						action: 'vfbp-rules-autocomplete',
						term: term,
						field_id: field,
					...
					function( data ) {
						...
						for ( var i = 0; i < data.length; i++ ) {
							var haystack = data[i].toLowerCase();
			...
			minChars: 1,
			menuClass: 'vfbp-rules-autocomplete'
		});
	}
});
