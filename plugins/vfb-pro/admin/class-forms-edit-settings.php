...
class VFB_Pro_Forms_Edit_Settings {
	...
	public function display() {
		...
		$disable_save_entry = isset( $data['data']['disable-save-entry'] ) ? $data['data']['disable-save-entry'] : '';
			...
			<div class="vfb-edit-section-inside">
				...
				<table class="form-table">
					<tbody>
						...
						<tr valign="top">
							<th scope="row">
								<label for="disable-save-entry"><?php esc_html_e( 'Disable Saving Entry', 'vfb-pro' ); ?></label>
							</th>
							<td>
								<fieldset>
									<label>
										<input type="hidden" name="disable-save-entry" value="0" /> <!-- This sends an unchecked value to the meta table -->
										<input type="checkbox" name="disable-save-entry" id="disable-save-entry" value="1"<?php checked( $disable_save_entry, 1 ); ?> /> <?php _e( "Disables saving entry data for each submission after all emails have been sent.", 'vfb-pro' ); ?>
									</label>
								</fieldset>
								<p class="description"><?php _e( 'This setting is most helpful for European Union users wishing to comply with the General Data Protection Regulation (GDPR).' , 'vfb-pro'); ?></p>
							</td>
						</tr>
