...
class VFB_Pro_Entries {
	...
	public function __construct() {
		...
		// Alter query to allow columns to be properly sorted
	...
	public function sortable_columns() {
		...
		foreach ( $fields as $slug => $c ) {
			// Add all fields, other than the checkbox
			if ( $slug != 'cb' ) {
				$columns[ $slug ] = $slug;
			}

			// Make sure Entry Date is sorted as a standard WP post_date
			if ( 'entry-date' == $slug ) {
				$columns[ $slug ] = 'date';
			}
		}
	...
	public function sortable_columns_orderby( $query ) {
		...
		if ( strpos( $orderby, '-field-' ) !== false ) {
			$field_id = str_replace( 'form-' . $form_id . '-field-', '', $orderby );
